...
def compare_images(img_file0, img_file1, structural_similarity=0.95):
    """
    Function for comparing two images provided in the filename strings, *img_file0* and *img_file1*.
    By default the structural similarity index measure (SSIM) is used, the lower limit of the
    index can be set using *structural_similarity*. It must be in the range [0,1], where one is a
    perfect match and zero is nothing matching.

    This function operates in a return code mode, thus a return value of 0 indicates that the
    images are the same, within the tolerance of the supplied method. A non-zero return code
    indicates that images are different.

    """
    data0 = skimage.io.imread(img_file0)
    data1 = skimage.io.imread(img_file1)

    retcode = 0
    if structural_similarity is not None:
        ssim = skimage.metrics.structural_similarity(data0, data1, multichannel=True)
        retcode = ssim < structural_similarity
    else:
        raise RuntimeError("No comparison performed, available options include: 'structural_similarity'")
    return retcode
